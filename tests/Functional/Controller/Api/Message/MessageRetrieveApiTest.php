...
class MessageRetrieveApiTest extends WebTestCase
{
    ...
    public function testApiCannotGetInterlocutorMessagesAnonymous(): void
    {
        $user = $this->getUserByUsername('JohnDoe');

        $this->client->request('GET', '/api/users/'.$user->getId().'/messages');
        self::assertResponseStatusCodeSame(401);
    }

    public function testApiCannotGetInterlocutorMessagesWithoutScope(): void
    {
        self::createOAuth2AuthCodeClient();
        $user = $this->getUserByUsername('JohnDoe');
        $messagingUser = $this->getUserByUsername('JaneDoe');
        ...
        $this->client->request('GET', '/api/users/'.$messagingUser->getId().'/messages', server: ['HTTP_AUTHORIZATION' => $token]);
        self::assertResponseStatusCodeSame(403);
    }

    public function testApiCanGetInterlocutorMessages(): void
    {
        self::createOAuth2AuthCodeClient();
        $user = $this->getUserByUsername('JohnDoe');
        $messagingUser = $this->getUserByUsername('JaneDoe');
        $otherUser = $this->getUserByUsername('JenaDoe');
        ...
        $dto = new MessageDto();
        $dto->body = 'some message';
        $messageManager->toThread($dto, $otherUser, $user);
