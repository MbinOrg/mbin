// adapted from https://codyhouse.co/ds/components/app/popover
...
function getPositionTarget(popover) {
    ...
    const positionTargetSelector = popover.element.getAttribute('data-position-target');
...
function initPopover(popover) {
    ...
    // init aria-labels
    for (let i = 0; i < popover.trigger.length; i++) {
...
function positionPopover(popover) {
    ...
    const selectedTriggerPosition = (popover.positionTarget) ? popover.positionTarget.getBoundingClientRect() : popover.selectedTrigger.getBoundingClientRect();
    ...
    const maxHeight = menuOnTop ? selectedTriggerPosition.top - popover.viewportGap : window.innerHeight - selectedTriggerPosition.bottom - popover.viewportGap;

    let initialStyle = popover.element.getAttribute('style');
    if (!initialStyle) {
        initialStyle = '';
    }
...
function getFocusableElements(popover) {
    ...
    const allFocusable = popover.element.querySelectorAll(focusableElString);
    ...
    if (0 < popovers.length) {
        ...
        for (let i = 0; i < popovers.length; i++) {
            (function(i) {
                ...
                const scrollableElement = popovers[i].getAttribute('data-scrollable-element');
        ...
        for (let j = 0; j < scrollingContainers.length; j++) {
            const scrollingContainer = document.querySelector(scrollingContainers[j]);
