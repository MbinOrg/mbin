...
15. Maintainers SHOULD NOT merge patches (PRs), even their own, unless there is at least one (1) other Maintainer approval.
    Or in exceptional cases, such as non-responsiveness from other Maintainers for an extended period (more than 3-4 days). And the patch / PR has a high critically level and cannot be waited for more than 4 days before being merged.
...
17. Maintainers SHOULD merge patches (PRs) from other Contributors. Since Contributors do NOT the rights to merge Pull Requests.
