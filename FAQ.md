...
**NO!** Try to avoid running development mode when you are hosting our own _public_ instance. Running in development mode can cause sensitive data to be leaked, such as secret keys or passwords (eg. via development console). There is no  rotation enabled in development mode and development mode will  a lot of messages to disk (incl. stacktraces).
