...
class UserRotatePrivateKeys extends Command
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        if ($username) {
            ...
            // all local users
            $users = $this->userRepository->findBy(['apId' => null]);
        } else {
            // unreachable
        ...
        $io->info('Successfully rotated the private key for '.$userCount.' users. It might take up to 24hours for other software to get the new public keys.');
